...
public class FeedCommand extends AbstractCommand {
    public FeedCommand(DiscordBot bot) {
        super(bot);

        this.name = "feed";
        this.help = "Nouris un amis";
        this.category = CommandCategory.FUN;
        this.options = Collections.singletonList(new OptionData(OptionType.USER, "friend", "The friend you want to feed", true));
    }

    @Override
    public void execute(SlashCommandInteractionEvent event) {
        try {
            HttpResponse<JsonNode> json = Unirest.get("https://nekos.life/api/v2/img/feed").asJson();
            User user = event.getOption("friend").getAsUser();
            event.replyEmbeds(new EmbedBuilder()
                    .setAuthor("Owo, " + event.getMember().getEffectiveName() + " just fed " + event.getGuild().getMember(user).getEffectiveName() + " !", event.getJDA().getSelfUser().getAvatarUrl())
