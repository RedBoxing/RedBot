...
class Menu extends events_1.default {
    ...
    constructor(channel, userID, pages, ms = 180000) {
        ...
        if (!this.channel) {
            this.channel.client.users.cache.get(this.userID).createDM();
        }
        ...
        if (missingPerms.length)
            console.log(`\x1B[96m[discord.js-menu]\x1B[0m Looks like you're missing ${missingPerms.join(', ')} in #${this.channel.name} (${this.channel.guild.name}). This perm is needed for basic menu operation. You'll probably experience problems sending menus in this channel.`);
        /*  } else {
    ...
    start() {
        ...
        this.channel.send(this.currentPage.content).then(menu => {
            ...
            logger.error(`\x1B[96m[discord.js-menu]\x1B[0m ${error.toString()} (whilst trying to send menu message) | You're probably missing 'SEND_MESSAGES' or 'EMBED_LINKS' in #${this.channel.name} (${this.channel.guild.name}), needed for sending the menu message.`);
        });
    }
    /**
        ...
        if (this.menu) {
            return new Promise((resolve, reject) => {
                resolve(this.menu.reactions.removeAll().catch(error => {
                    ...
                    logger.error(`\x1B[96m[discord.js-menu]\x1B[0m ${error.toString()} (whilst trying to remove message reactions) | You're probably missing 'MANAGE_MESSAGES' in #${this.channel.name} (${this.channel.guild.name}), needed for removing reactions when changing pages.`);
                }));
            });
        }
    }
        ...
        for (const reaction in this.currentPage.reactions) {
            await this.menu.react(reaction).catch(error => {
                if (error.toString().indexOf('Unknown Emoji') >= 0) {
                    logger.error(`\x1B[96m[discord.js-menu]\x1B[0m ${error.toString()} (whilst trying to add reactions to message) | The emoji you were trying to add to page "${this.currentPage.name}" (${reaction}) probably doesn't exist. You probably entered the ID wrong when adding a custom emoji.`);
                }
                else {
                    logger.error(`\x1B[96m[discord.js-menu]\x1B[0m ${error.toString()} (whilst trying to add reactions to message) | You're probably missing 'ADD_REACTIONS' in #${this.channel.name} (${this.channel.guild.name}), needed for adding reactions to the page.`);
                }
            });
        }
    }
